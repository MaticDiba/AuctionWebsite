...
namespace AuctionWeb.Helpers
{
    public class FilesHelper
    {
        ...
        public JsonFiles GetFileList()
        {
            ...
            if (Directory.Exists(fullPath))
            {
                ...
                foreach (FileInfo file in dir.GetFiles())
                {
                    ...
                    r.Add(UploadResult(0, file.Name, SizeInt, file.FullName));
                }
        ...
        internal object GetFileList(List<Image> images)
        {
            var r = new List<ViewDataUploadFilesResult>();
            foreach(Image im in images)
            {
                try
                {
                    long length = new System.IO.FileInfo(im.Path).Length;
                    r.Add(UploadResult(im.ImageId, im.Name, (int)length, im.Name));
                }
                catch
                {
                    r.Add(UploadResult(im.ImageId, im.Name, 1000, im.Name));
                }
            }
            JsonFiles files = new JsonFiles(r);
        ...
        private void UploadWholeFile(HttpContextBase requestContext, List<ViewDataUploadFilesResult> statuses)
        {
            ...
            for (int i = 0; i < request.Files.Count; i++)
            {
                ...
                statuses.Add(UploadResult(0, file.FileName, file.ContentLength, file.FileName));
            }
        }

        private void Resize(string fullPath, int _width = 600)
        {
            System.Drawing.Image imgOriginal = System.Drawing.Image.FromFile(fullPath);
            this.Width = _width;

            float sourceWidth = imgOriginal.Width;
            float sourceHeight = imgOriginal.Height;
            float destHeight = 0;
            float destWidth = 0;
            int sourceX = 0;
            int sourceY = 0;
            ...
            grPhoto.DrawImage(imgOriginal,
                new System.Drawing.Rectangle(destX, destY, (int)destWidth, (int)destHeight),
            ...
            using (MemoryStream memory = new MemoryStream())
            {
                ...
                {
                    bmPhoto.Save(memory, System.Drawing.Imaging.ImageFormat.Jpeg);
        ...
        public ViewDataUploadFilesResult UploadResult(int ImageId, String FileName, int fileSize, String FileFullPath)
        {
